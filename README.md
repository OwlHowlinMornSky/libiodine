...
Libiodine is a simple library performing JPEG, PNG, WebP, TIFF and GIF (partial) compression/optimization written in
...
Libiodine exposes several functions for compressing and converting images, both from files and in-memory buffers.
...
Libiodine exposes C functions, auto-detecting the input file type:
...
`cbindgen --config cbindgen.toml --crate libiodine --output include/libiodine.h`
...
Libiodine also supports optimization. This performs a lossless process, resulting in the
