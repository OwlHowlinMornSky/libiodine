    ...
    let encoded_image_memory = if is_animated {
        ...
        for (i, f) in frames.into_iter().enumerate() {
            if must_resize {
                ...
                dyn_image = resize_image_n(dyn_image, parameters.allow_magnify, parameters.width, parameters.height);
                if i == 0 {
        ...
        if must_resize {
            input_image = resize_image_n(input_image, parameters.allow_magnify, parameters.width, parameters.height);
        }
